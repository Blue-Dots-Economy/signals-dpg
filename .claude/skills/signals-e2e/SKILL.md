---
name: signals-e2e
description: Bring the local Signals DPG stack up (or reuse a live one), run the black-box Playwright suite end to end with every local capability enabled (email/SMS oracle, search stub, item_search indexer), and produce a five-section signoff a human can read. Use when asked to end-to-end test Signals, run a full e2e, test the U18/guardian flow, sign off on Signals before a release, or "run /signals-e2e".
---
...
| `aggregator`, `admin` | 13 | Journey I, J, V |
...
| 1 | Reuse or bring up the stack (`lib/stack-up.sh`), start the 3 stubs, snapshot the DB | `lib/run.sh` |
...
| Map count < list total | *Expected*, not a bug: un-geocoded migrated rows plus the map's viewport scope. Asserted as `@known` in section 3, never section 2. |
