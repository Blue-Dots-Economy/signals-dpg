...
**`AUTH_PROVIDER` gates a second identity provider on the session path.** The Keycloak migration (`docs/superpowers/plans/2026-07-23-keycloak-migration-design.md`) is landing behind this flag, resolved into `authConfig.provider` / `.keycloak_enabled` / `.betterauth_enabled` in `apps/api/src/config.ts`. **Default `betterauth` — everything below is dormant, and the session path is byte-for-byte today's behaviour.**

`auth_middleware` and `validate_session` both delegate to `resolveKeycloakSession` (`plugins/auth/resolve_session.ts`) before falling back to better-auth, so the two cannot drift:

- **`dual`** — a bearer token whose `iss` matches the configured realm is validated as a Keycloak JWT (`src/utils/keycloak_token.ts`) and resolved to a local `user` row by `provisionUserFromClaims` (`src/services/auth/provisioning.ts`). Anything else falls through to better-auth. A token that *looks* Keycloak-issued but fails validation is **not** retried against better-auth.
