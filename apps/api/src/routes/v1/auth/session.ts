...
import { authConfig, instance } from '@/config';
import {
  buildEndSessionUrl,
  exchangeCode,
  idTokenNonce,
...
import { consumeFlowState } from '@/services/auth/oidc_flow_state';
import {
  clearFlowCookie,
  FLOW_COOKIE,
  requestOrigin,
  startLoginFlow,
} from '@/routes/v1/auth/login_flow';
import {
  createSession,
  destroySession,
  newCsrfToken,
    ...
    handler: async (request, reply) => {
      ...
      return reply.redirect(
        await startLoginFlow(request, reply, {
          ...
          appOrigin: request.query.appOrigin,
        })
      );
    },
  });
