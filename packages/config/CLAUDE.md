...
- `CONSENT_CONFIG_SOURCE` accepts only `local`. `remote` used to parse and then return `[]` — an unimplemented stub that silently emptied consent config for every network if an operator set it. Remote consent loading, if built, should land as a real branch rather than a reachable no-op.
