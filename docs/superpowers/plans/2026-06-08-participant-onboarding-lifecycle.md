...
> **Rescope (2026-06-08):** Fresh implementation on `feat/participant-onboarding-lifecycle`, branched off `feature`. Supersedes the draft branch `feat/onboarding-lifecycle` (referenced, not merged). All 13 anchor files + the shape-only validation helper (`packages/schemas/src/network_workflow.ts`) were verified present on `feature`; task line numbers are indicative — confirm against the live file when executing each task.
...
- Working dir for every command in this plan is the worktree root: `/Users/ASUS/Documents/workspace/bluedots-economy/Signals-DPG/.claude/worktrees/participant-onboarding-lifecycle`.
