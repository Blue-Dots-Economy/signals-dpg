...
- **Scoping (BOTH modes use the same per-item ownership source — `item_metrics.onboarded_by_org_id`):** ownership means "this org onboarded this item," identically in both modes, so an aggregator can never receive decrypted PII for an item another org onboarded.
  - `item_ids` mode joins `items`↔`item_metrics` and, for `org_type === 'aggregator'`, adds `eq(item_metrics.onboardedByOrgId, acting_org.org_id)` to the WHERE (filtered by `item_id IN (requested)`). This is the **same table the dashboard scopes by**, so "decryptable" == "visible on your dashboard." For `network_service`, drop that clause (keep all).
  - `user_id` mode uses the **same join + same `item_metrics.onboarded_by_org_id` filter**, keyed by `items.created_by == user_id` instead of `item_id IN (...)`. It deliberately does NOT gate on `user.onboarded_by_org_id`: per-user ownership would diverge from `item_ids` mode and could leak another org's item for a shared user. `network_service` drops the org filter (keep all).
...
- **`user_id` path:** same `items`↔`item_metrics` join + decrypt as the item_ids path, keyed by `items.created_by == user_id` with the per-item `item_metrics.onboarded_by_org_id` filter for aggregators; `skipped` is `[]`.
