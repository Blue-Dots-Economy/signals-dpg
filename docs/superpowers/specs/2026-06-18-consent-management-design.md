...
- **Idempotent accept (no duplicate rows on double-click).** `POST /consent/accept` does a **read-check-write in one transaction**: read the latest event for (user, network, doc_type); if it's already an `accepted` against the active version, no-op; else insert. (Belt-and-suspenders: a partial unique index can forbid a second `accepted` row for the same (user, network, doc_type, document_version).)
