...
The instinct is to add the limiter in the Fastify service and key it on `request.ip`. The proxy chain is **AWS Classic ELB (L4/TCP, PROXY protocol) → Kong (sole ingress controller) → API pods**, and with `trustProxy: true` (set at `apps/api/src/app.ts:105`) Fastify sets `request.ip` to the **left-most `X-Forwarded-For` entry**. That has one *certain* problem and one *possible* one:

- **Certain — spoofable, therefore bypassable.** `trustProxy: true` trusts *every* hop, so the left-most XFF is whatever the client wrote there. An abuser sets and rotates `X-Forwarded-For` per request; each request looks like a new IP and the per-IP counter never accumulates. Behind an L4 LB, XFF is client-settable and unsanitised, so this is not theoretical — the platform team documented it verbatim in `bluedots-automation/helm/common-services/values.yaml`: *"every request looked like a new IP and the rate limiter [was] fully bypassable."* Since rate limiting exists to stop abusers, a control defeated by adding one header is not a control.
- **Possible — collapse to an internal IP.** If no XFF reaches the pod, `proxy-addr` falls back to the socket peer (Kong's pod IP) and every user collapses into **one** bucket. Whether this happens depends on Kong's exact XFF forwarding, so it is a risk rather than a guarantee — for honest traffic `request.ip` is often the user's real IP.

The point is not that `request.ip` is *always* wrong — for honest users it is usually their real IP — but that it is **untrustworthy for enforcement**, which is the only thing that matters for a throttle. The non-client-settable client IP comes from the **PROXY-protocol header** the ELB prepends at the connection layer, trusted only from the in-VPC LB (`real_ip_header: proxy_protocol`, `trusted_ips: <VPC CIDR>`), and is resolved **at Kong**. So Kong is where the throttle belongs.

This also matches how the service itself already treats `request.ip`: the two existing in-app per-IP limiters — `routes/v1/auth/u18_precheck.ts` and `routes/v1/consent/u18_signup_guardian.ts` — are explicitly soft signals, not enforcement (*"a hint, not a security control"*, *"a first line … before the per-scope / per-guardian-contact caps"*). And it matches the platform's stated intent — the signals-api catch-all tier `rl-signals-api: { minute: 10000 }` carries the comment *"loose catch-all; per-API limits do the real throttling."*

> **Verifying the exact value (optional):** `request.ip`'s concrete value in a given environment can be confirmed empirically — Fastify logs `remoteAddress` (= `request.ip`) per request under `logger: true`, so the deployed API pod logs already reveal whether values are diverse real IPs or collapsed to one; a one-off debug log of `request.ip` + `x-forwarded-for` + `x-real-ip` on `/discover` shows exactly what Kong forwards. Regardless of that value, the spoofability above stands, so the conclusion (throttle at Kong) is unchanged.
