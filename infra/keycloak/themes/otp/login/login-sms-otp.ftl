...
<!-- template: login-sms-otp.ftl -->
    ...
    <#if section="header">
        ${msg("smsOtpTitle")}
    <#elseif section="form">
        <form id="kc-sms-otp-form"
              class="${properties.kcFormClass!} bd-otp-form"
              action="${url.loginAction}"
              method="post"
              novalidate="novalidate">
            <div class="${properties.kcFormGroupClass!}">
                <label class="${properties.kcLabelClass!}" for="bd-otp-0">${msg("smsOtpLabel")}</label>
                <div class="bd-otp-grid" role="group" aria-label="${msg('smsOtpLabel')}">
                    <#list 0..5 as i>
